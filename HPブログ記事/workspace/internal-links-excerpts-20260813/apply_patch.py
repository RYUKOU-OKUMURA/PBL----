#!/usr/bin/env python3
"""Safely patch published posts 965, 612, 976 (excerpt + internal links)."""
...
LINK_1999 = (
    '<p>手術後の生活動作で「できないこと」が気になる場合は、'
    '<a href="https://physical-balance-lab.com/1999/">'
    "側弯症 手術後 できないことは何？大人が生活動作で確認したい注意点"
    "</a>もあわせてお読みください。</p>"
)
LINK_2079 = (
    '<p>学校・体育・スポーツ復帰の目安は、'
    '<a href="https://physical-balance-lab.com/2079/">'
    "側弯症手術後のリハビリ期間は？学校・体育・スポーツ復帰の目安"
    "</a>で整理しています。</p>"
)
RELATED_EXTRA = (
    '<li><a href="https://physical-balance-lab.com/1999/">'
    "側弯症 手術後 できないことは何？大人が生活動作で確認したい注意点"
    "</a></li>\n"
    '<li><a href="https://physical-balance-lab.com/2079/">'
    "側弯症手術後のリハビリ期間は？学校・体育・スポーツ復帰の目安"
    "</a></li>\n\n"
)

MARKER_1999 = (
    "気になる症状がある場合は、自己判断で原因を決めず、担当医と相談することが大切です。</p>"
)
MARKER_2079 = (
    "急な悪化や新たなしびれなどがある場合は、早めの受診を検討してください。</p>"
)
RELATED_MARKER = "<h2>関連記事</h2>\n<ul>\n\n"
...
def transform_content(post_id: int, content: str) -> str:
    if post_id in (965, 612):
        return content
    if post_id != 976:
        raise PatchError(f"Unsupported post_id: {post_id}")
    updated = content
    if LINK_1999 not in updated:
        if MARKER_1999 not in updated:
            raise PatchError("976: 1999 insertion marker not found")
        updated = updated.replace(MARKER_1999, MARKER_1999 + "\n" + LINK_1999, 1)
    if LINK_2079 not in updated:
        if MARKER_2079 not in updated:
            raise PatchError("976: 2079 insertion marker not found")
        updated = updated.replace(MARKER_2079, MARKER_2079 + "\n" + LINK_2079, 1)
    if RELATED_MARKER not in updated:
        raise PatchError("976: related-article marker not found")
    related = updated.split("<h2>関連記事</h2>", 1)[-1]
    if "physical-balance-lab.com/1999/" not in related:
        updated = updated.replace(RELATED_MARKER, RELATED_MARKER + RELATED_EXTRA, 1)
    if "physical-balance-lab.com/1999/" not in updated or "physical-balance-lab.com/2079/" not in updated:
        raise PatchError("976: expected internal links missing after transform")
    return updated
...
def write_qa_preview(post_id: int, backup: dict[str, Any], expected: dict[str, str]) -> Path:
    ...
    if post_id == 976:
        body += "## New internal-link fragments\n\n"
        body += LINK_1999 + "\n\n" + LINK_2079 + "\n\n"
        body += "## Related list additions\n\n" + RELATED_EXTRA + "\n"
...
def command_plan(_config: dict[str, str]) -> None:
    ...
    for post_id in POST_IDS:
        ...
        print(f"PLANNED post={post_id} excerpt_changed={expected['excerpt'] != backup.get('excerpt', '')} content_changed={expected['content'] != backup['content_raw']} qa={qa_path}")
    ...
    try:
        for post_id in POST_IDS:
            ...
            if updated["excerpt"]["raw"].strip() != expected["excerpt"].strip():
                raise PatchError(f"post {post_id}: excerpt not applied correctly")
